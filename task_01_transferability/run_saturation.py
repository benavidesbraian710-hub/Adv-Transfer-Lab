...
def run(config, dataset, src_model, tgt_model, device):
    ...
    pbar = tqdm(steps, desc=f"📈 {config['name']}", dynamic_ncols=True)
    for step in pbar:
        s_list, t_list = [], []
        for t in range(config['trials']):
            pbar.set_description(f"Saturation: Step {step} | Trial {t + 1}")
        ...
        pbar.set_postfix({"Src": f"{np.mean(s_list):.1f}%", "Tgt": f"{np.mean(t_list):.1f}%"})